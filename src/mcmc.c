...
int             *tempId = NULL;              /* inverse of chainId, i.e. temp -> chainNum. HARRY */
...
#   else
    ...
    if (RandomNumber(seed) < r)
        ...
        chainId[swapB] = tempX;
        /* Begin HARRY */
        tempId[chainId[swapB]] = swapB;
        tempId[chainId[swapA]] = swapA;
        /* End HARRY */
    ...
    if (expecting == Expecting(PARAMETER))
        ...
        /* set Nchains (numChains) ************************************************************/
            ...
            if (expecting == Expecting(EQUALSIGN))
                ...
                expecting = Expecting(PARAMETER) | Expecting(SEMICOLON);
                /* Begin Angel */
                if (chainParams.readTemp == YES)
                    {
                    MrBayesPrint ("%s   Introduce temperature file name:\n", spacer);
                    if (scanf("%s", nameFileTemp)==1)
                        {
                        f=fopen(nameFileTemp,"r");
                        MrBayesPrint ("%s   \n", spacer);
                        }
                    else
                        {
                        MrBayesPrint ("%s   \n", spacer);
                        MrBayesPrint ("%s   Name incorrect\n", spacer);
                        return (ERROR);
                        }
                    if(f!=NULL)
                        MrBayesPrint ("%s   Read file name correct\n", spacer);
                    else
                    {
                        MrBayesPrint ("%s   Read file name incorrect\n", spacer);
                        return(ERROR);
                    }
                    i=0;
                    while (i<chainParams.numChains)
                    {
                        if (fgets(num, sizeof(num), f)!=NULL)
                        {
                        chainParams.tempValue[i]=atof(num);
                        i++;
                        } else 
                        {
                        MrBayesPrint ("%s   File incorrect\n", spacer);
                        return(ERROR);
                        }
                    }
                    quicksort(chainParams.tempValue,0,chainParams.numChains-1);
                    }
                /* End Angel */
                }
            else
        ...
        /* Begin Angel */
        /* set readTemp (chainParams.readTemp) *********************************************/
        else if (!strcmp(parmName, "ReadTemp"))
            {
            if (expecting == Expecting(EQUALSIGN))
                expecting = Expecting(ALPHA);
            else if (expecting == Expecting(ALPHA))
                ...
                if (IsArgValid(tkn, tempStr) == NO_ERROR)
                    {
                    if (!strcmp(tempStr, "Yes"))
                        chainParams.readTemp = YES;
                    else
                        chainParams.readTemp = NO;
                    }
                else
                    {
                    MrBayesPrint ("%s   Invalid argument for ReadTemp\n", spacer);
                    free(tempStr);
                    return (ERROR);
                    }
                if (chainParams.readTemp == YES)
...
/* Begin Harry */
void GetSeqSwappers (int *swapA, int *swapB, int temp, int run)
{
    int i = run * chainParams.numChains;
    (*swapA) = tempId[temp+i];
    (*swapB) = tempId[temp+i+1];
    return;
}
...
#   else
    ...
    if (numLocalChains == 1)
        ...
        i = 0;
        /* Begin HARRY */
        // see any line in this file with GetTree (m->brlens, coldId, state[coldId]);
        // or see WriteTopologyToFile in sumpt.c
        f = fopen("./results/trees.txt", "a");
        ModelInfo *m = &modelSettings[0];
        Tree *tree = GetTree(m->brlens, 1, state[1]); 
        WriteTopologyToFile(f,tree->root->left, tree->isRooted);
        MrBayesPrintf (f, ";\n");
        fclose(f);

        f = fopen("./results/temp.out", "a");
        for (chn = 0; chn < numLocalChains; chn++) {
                fprintf(f, "%d ", chainId[chn]);
        }
        fprintf(f,"\n");
	    fclose(f);
        /* End HARRY */
        for (chn=0; chn<numLocalChains; chn++)
            {
            tempE[chainId[chn]] = curLnL[chn]; // HARRY
            if (i >= chainParams.printMax)
                {
                if (i == chainParams.printMax)
                    MrBayesPrint (".. ");
    ...
    /* Begin HARRY */
    f = fopen("./results/energy.out", "a");
    for (chn = 0; chn < numLocalChains; chn++) {
        fprintf(f, "%1.3lf ", tempE[chn]);
    }
    fprintf(f, "\n");
    fclose(f);
    /* End HARRY */
...
#   endif
    ...
    if (memAllocs[ALLOC_CHAINID] == YES)
        ...
        nErrors++;
             ...
             (tempId = (int *)SafeMalloc((size_t)numLocalChains * sizeof(int))) == NULL) // HARRY
        {
        ...
        if (chainParams.numChains > 1 && n % chainParams.swapFreq == 0)
            ...
            for (i = 0; i<chainParams.numRuns; i++)
                ...
                if (chainParams.swapAdjacentOnly == NO) {
                    ...
                    for (j = 0; j<chainParams.numChains - 1; j++) {
                        GetSeqSwappers(&swapA, &swapB, j, i);
                        if (AttemptSwap (swapA, swapB, seed) == ERROR) {
                            MrBayesPrint ("%s   Unsuccessful swap of states\n", spacer);
                            return ERROR;
                        }
...
#   endif
        ...
        /* Begin Angel */
        if (n % chainParams.swapFreq == 0)
        {
        file1 = fopen("./results/tempIndex.dat", "a");
        file2 = fopen("./results/energyValue.dat", "a");
        fprintf(file1, "%d", n);
        fprintf(file2, "%d", n);
        for (chn = 0; chn < numLocalChains; chn++) 
        {
            fprintf(file1, "\t%d", chainId[chn]-chainParams.numChains*(chainId[chn]/chainParams.numChains));
            fprintf(file2, "\t%f", curLnL[chn]);
        }
        fprintf(file1,"\n");
        fprintf(file2,"\n");
	    fclose(file1);
...
MrBFlt Temperature (int id)
{
    ...
    {
        id %= chainParams.numChains;
        return (chainParams.tempValue[id]);  
    }
    /* End Angel */
}
